...
In practice, users often start from the tree and then add associated metadata to the right-hand side. This workflow is also supported. The `width` argument always specifies the relative width of the inserted plot with respect to the main plot. In the following example, the tree is the main plot and the metadata panel is set to half of the tree width.
...
tree_main %>% insert_right(metadata_main, width = .5)
```

If you prefer to think from the metadata plot first, the following code produces the same layout with the tree inserted on the left.

```{r fig.height=4, fig.width=7}
metadata_main %>% insert_left(tree_main, width = 2)
```

The space between aligned panels can also be controlled globally with `options(space.between.plots = ...)`. The value is interpreted in millimeters. This is useful when two panels are technically aligned but you still want a small visual gap between them.

```{r fig.height=4, fig.width=7}
old_space <- getOption("space.between.plots")
on.exit(options(space.between.plots = old_space), add = TRUE)
options(space.between.plots = 2)
