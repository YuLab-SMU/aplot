...
In practice, users often start from the tree and then add associated metadata to the right-hand side. Conceptually, the `width` argument always specifies the relative width of the inserted plot with respect to the main plot. In current usage, the reliable way to obtain such a layout is still to start from the metadata plot and insert the tree on the left. The following code produces a layout in which the metadata panel is half of the tree width.
...
The `set_panel_spacing()` function controls the gap between aligned panels in an `aplot` object. Numeric values are interpreted in millimeters and refer to the spacing between neighboring panels.
...
If the individual `ggplot` objects already define their own `plot.margin`, you can preserve these margins by setting the spacing mode to `"asis"`.
